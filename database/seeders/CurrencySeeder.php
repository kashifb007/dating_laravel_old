    ...
    public function run(): void
    {
        ...
        foreach ($currencies as $currency) {
            Currency::create([
                'code' => $currency['code'],
                'name' => $currency['name'],
                'symbol_left' => $currency['symbol'],
            ]);
        }

        //attach currencies to countries
        $GBcurrency = Currency::whereCode('GBP')->first();
        $GBcountry = Country::whereIso('GB')->first();

        $GBcurrency->countries()->attach($GBcountry->id);

        $UScurrency = Currency::whereCode('USD')->first();
        $UScountry = Country::whereIso('US')->first();

        $UScurrency->countries()->attach($UScountry->id);

        $IEcurrency = Currency::whereCode('EUR')->first();
        $IEcountry = Country::whereIso('IE')->first();

        $IEcurrency->countries()->attach($IEcountry->id);
    }
}
